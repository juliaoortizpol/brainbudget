...
export class SyncEngineService {
  ...
  async syncEmailsForUser(userId: string) {
    ...
    const connection = await this.integrationsService.getGmailConnection(userId);
    
    if (!connection || connection.status !== 'active' || !connection.refreshToken) {
      this.logger.warn(`User ${userId} does not have an active Gmail connection`);
    ...
    for (const config of configs) {
      for (const rule of config.parsingRules) {
        ...
        if (rule.subjectKeywords.length > 0) {
          query += ` subject:"${rule.subjectKeywords[0]}"`;
        }

        // Add time constraint
        if (connection.lastSyncedAt) {
          const timestamp = Math.floor(connection.lastSyncedAt.getTime() / 1000);
        ...
        try {
          ...
          this.logger.debug(`Found ${messages.length} messages for rule ${rule.parserCodeId}`);
          ...
          for (const msg of messages) {
            ...
            const exists = await this.transactionModel.exists({ externalId: msgId, userId: new Types.ObjectId(userId) });
            if (exists) {
            ...
            // Gmail body can be in parts
            let bodyData = '';
            if (msgResponse.data.payload?.parts) {
              const htmlPart = msgResponse.data.payload.parts.find(p => p.mimeType === 'text/html');
              if (htmlPart && htmlPart.body?.data) {
                bodyData = Buffer.from(htmlPart.body.data, 'base64').toString('utf-8');
              }
            } else if (msgResponse.data.payload?.body?.data) {
              bodyData = Buffer.from(msgResponse.data.payload.body.data, 'base64').toString('utf-8');
            }

            // 3. Parse Email
            const parsedResult = await this.emailParserService.parseEmail(sender, subject, bodyData);
            ...
            if (parsedResult) {
              ...
              await newTx.save();
              totalSaved++;
              this.logger.log(`Saved new transaction from email: ${parsedResult.transaction.description}`);
            }
          }
        } catch (error: any) {
          this.logger.error(`Error processing rule for ${config.name}: ${error.message}`);
        }
      }
    }
